...
class DDiTBlock(nn.Module):
  def __init__(self, dim, n_heads, cond_dim, mlp_ratio=4, dropout=0.1):
    ...
    # AdaLN modulation driven by the sigma (timestep) embedding.
...
class DDitFinalLayer(nn.Module):
  def __init__(self, hidden_size, out_channels, cond_dim):
    ...
    # Main projection for all but the last token id.
    self.linear = nn.Linear(hidden_size, out_channels - 1)
    self.linear.weight.data.zero_()
    self.linear.bias.data.zero_()
    # Separate projection for the extra (mask) token id.
  ...
  def forward(self, x, c):
    ...
    x_main = self.linear(x_mod)
    x_extra = self.linear_extra(x_mod)
    return torch.cat([x_main, x_extra], dim=-1)
